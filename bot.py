...
def check_arbitrage(pair):
    ...
    if profit_percent >= MIN_PROFIT_PERCENT and min_buy[1]['volume'] >= MIN_LIQUIDITY:
        msg = f"💰 Arbitraj Topildi!\nCoin: {pair}\n⬇️ Sotib olish: {min_buy[0]} @ {min_buy[1]['ask']}\n⬆️ Sotish: {max_sell[0]} @ {max_sell[1]['bid']}\n📈 Foyda: {round(profit_percent, 2)}%"
        send_message(msg)
    elif chesk_mode and profit_percent >= 1.0:
        msg = f"🧐 Chesk imkoniyat: {pair}\n⬇️ {min_buy[0]}: {min_buy[1]['ask']}\n⬆️ {max_sell[0]}: {max_sell[1]['bid']}\n📉 Foyda: {round(profit_percent, 2)}%"
        send_message(msg)
...
if __name__ == "__main__":
    threading.Thread(target=listen_bot).start()
    run_bot()